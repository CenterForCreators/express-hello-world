...
const PAY_DESTINATION = process.env.PAY_DESTINATION || "rsxUkmjnAn8PRDz8RYrPusb9mTDYn5NqG8";
  ...
  try {
    const link = await createXummPayload({
      txjson: {
        ...
        Amount: { currency: "CFC", issuer: PAY_DESTINATION, value: "10" }
      },
      options: {
        submit: true,
...
app.post('/api/join', async (req, res) => {
  ...
  try {
    const r = await fetch(scriptURL, { method: "POST", body: new URLSearchParams({ email }) });
    const j = await r.json();
