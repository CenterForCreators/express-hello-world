// ===== app.js (backend with Xumm iOS-safe auth + faucet + payments) ===== 
...
/* ---------- XUMM SIGN-IN (BACKEND POLLING, iOS-SAFE) ---------- */

// Helper: wrap XUMM Platform API
...
// Start login: create SignIn payload
  ...
  try {
    const r = await xummFetch('payload', {
      ...
      body: JSON.stringify({
        ...
        options: { submit: false } // IMPORTANT: no return_url for iOS
      })
    });
    const j = await r.json();
